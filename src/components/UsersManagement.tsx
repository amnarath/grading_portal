...
export default function UsersManagement() {
  ...
  const sendPaymentEmail = (entry: GradingEntry, userEmail: string) => {
    const totalAmount = Number(entry.price) + Number(entry.surcharge_amount);
    const subject = entry.surcharge_amount > 0 ? 'Surcharge Payment Required - Pikamon Grading' : 'Payment Required - Pikamon Grading';
    const body = `
Dear Customer,

Thank you for your grading submission. Please complete the payment using the following details:

Amount Due: €${totalAmount.toFixed(2)}
${entry.surcharge_amount > 0 ? `(Includes surcharge of €${Number(entry.surcharge_amount).toFixed(2)})` : ''}
...
Important: Please include the reference number in your transfer to ensure proper processing.

If you have any questions, please don't hesitate to contact us.

Best regards,
Pikamon Grading Team
    `.trim();
  ...
  const renderEntryRow = (entry: GradingEntry, userEmail: string) => (
        ...
        {renderEditableCell(entry, 'payment_status',
          <div className="flex items-center space-x-2">
            ...
            {(entry.payment_status === 'Unpaid' || entry.payment_status === 'Surcharge Pending') && (
              <button
                onClick={() => sendPaymentEmail(entry, userEmail)}
                            ...
                            {userEntries[user.id]?.loading ? (
                              ...
                              <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                  ...
                                  <tbody className="bg-white divide-y divide-gray-200">
                                    {userEntries[user.id]?.entries.map(entry => renderEntryRow(entry, user.email))}
                                  </tbody>
                                </table>
                              </div>
                            )}
